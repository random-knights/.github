...
<!-- The AiEDs section below is GENERATED and reports the WHOLE ORGANISATION.
     Everything between AIEDS:BEGIN and AIEDS:END is written by the AiEDs
     README generator from the SessionEnd ledgers and placed here by
     scripts/sync-aieds.mjs --repo organisation. Do not hand edit it: a typed
     figure on the org's public front page is a figure nobody can check, and
     the CI Gate fails a block that has drifted from the generated one. -->
