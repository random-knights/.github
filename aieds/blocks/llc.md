...
### 🌎 <span style="color:#EDC303"> Total **AiEDs** Usage | llc </span> 🏰
...
<sub>97.9 percent of our input is cache reads, so that rule decides the answer by 8.0x.
Weighting a cache read at 0.1 instead gives <b>2.3 kWh, 1.0 kg CO₂e, 17 days of Tree-Time</b>.
...
| --- | ---: | --- |
| Phone charges | 1,556 | 12 Wh per charge |
| LED bulb hours | 1,867 | 10 W bulb |
| Laptop hours | 373 | 50 W laptop |
| Driving distance | 47 km | 170 g CO₂e per km |
| Tree-Time | 139 days | 21 kg CO₂e per mature tree per year |
...
**Energy cost: USD 3.42.** 18.7 kWh at USD 0.1834 per kWh, the United States average residential price for June 2026 (18.34 cents per kilowatthour), from <a href="https://www.eia.gov/electricity/monthly/epm_table_grapher.php?t=epmt_5_3">U.S. Energy Information Administration, Electric Power Monthly, Table 5.3</a>. The rate is pinned, not looked up at render time, so this figure is reproducible.

**Modeled provider spend: USD 65.86.** The same sessions priced at published API list prices, rates version 2026-09-01, with cache writes at 1.25x and cache reads at 0.1x an input token. It is a MODEL, not a bill: this work runs on a subscription, so the marginal cost was nothing. It covers the 13 of 17 sessions counted above whose model that file prices; the other 4 carry a model nobody has priced and add nothing, rather than an assumed rate.

**Trees needed: 1.** 8.0 kg CO₂e divided by 21 kg CO₂e, the yearly capture of one mature tree, rounded up: 1 mature tree would absorb this carbon within one year. Put the other way round, that is the Tree-Time above: one mature tree working for 139 days.
