...
- `rk_ui` depends on `rk_branding` for shared color tokens.
...
- It includes app-specific surface decisions such as scaffold background,
  ...
  overlay style.
- Moving it into `rk_branding` would mix tokens with app theme application.
...
- `rk_branding`: color tokens, font helpers, brand constants, typography tokens.
...
- Theme token names that are not app-runtime coupled.
