...
App identity should not include secrets, Firebase options, environment values,
...
secret/test access, or user permissions by itself. Apps remain responsible for
...
- the full primary workflow of an app
- detailed settings
...
- the main workflow
- workflow-specific state and controls
...
| `supportsTest` | App has a local Test/secret/lab destination. |
...
secret/auth visibility. Apps must provide callbacks when they wire the drawer.
...
- Full weather workflow, if it becomes a tool, belongs under More.
