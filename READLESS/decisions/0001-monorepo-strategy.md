...
- Asset paths and Firebase environment configuration need careful handling
  during extraction.
- Generated files, build outputs, and local tool artifacts must remain
...
2. Add workspace tooling that can support apps and packages without changing app
   behavior.
3. Confirm `.gitignore` covers generated artifacts, build outputs, Firebase
   caches, and dependency folders.
...
6. Extract `rk_ai` contracts after pure shared types are stable, while keeping
   provider contracts and privileged calls in Firebase Functions.
