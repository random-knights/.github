...
- Local emulator and local agent workflows can be standardized across apps.
...
- Asset paths and Firebase environment configuration need careful handling
  during extraction.
- Generated files, secrets, build outputs, and local tool artifacts must remain
...
2. Add workspace tooling that can support apps and packages without changing app
   behavior.
3. Confirm `.gitignore` covers generated artifacts, secrets, build outputs,
...
6. Extract `rk_ai` contracts after pure shared types are stable, while keeping
   provider secrets and privileged calls in Firebase Functions.
...
8. Add CI checks for Flutter analysis, package tests, Firebase Functions builds,
   rules tests, formatting, and secret scanning.
