...
**Last updated:** 2026-06-19 (session 49 — 2D/3D toggle + renderer shipped (`0daaf35`); lifecycle fix (`99e351f`); earth2d slot-parity partial (composite+region merged; full parity on branch); score v0.3 S1–S4 CLIENT MERGED (`ab15e4c`) — `earthHealthScoreRefresh` FUNCTION-DEPLOY PENDING (SST=0 live gap); liveReady manifest (`38f436b`); BAA layer (`7e7cd00`); SSTA overlay (`4c22c06`); Waves flow layer (`93e7fb5`); filter parity batch 1 SHIPPED (`ccb72b9`+); CMEMS currents worker merged (`ad16eec`); two-channel submit (GitHub Discussions + Pro Test) on branch — owner GitHub-setup pending; origin/main = `64d6761` (git-verified))
...
| --- | --- | --- | --- | --- |
| Earth | `deve10per` / dev-kitt | `apps/rand0m` (integration only — §22) | main=`64d6761`; hosting=`5df0df4` (deployed) | Earth features, layers, governance; catalog owner; integrator/deploy; absorbs Systems+Connect scopes |
...
_Position: **wf90 DEPLOYED `5df0df4` (2026-06-18); 2D/3D renderer toggle SHIPPED + lifecycle fixed; score v0.3 CLIENT merged — function-deploy PENDING (SST=0 live gap); filter parity batch 1 SHIPPED; CMEMS/SSTA/BAA/Waves on main; earth2d slot-parity partial; ⭐⭐ NORTH-STAR: RETIRE CESIUM → full 2D-canvas.** `origin/main` (xyz) = `64d6761` (git-verified, session 49); last confirmed hosting deploy = `5df0df4` (wf90, 2026-06-18; multiple commits PENDING next wf90 + owner device-pass + earth-worker redeploy + owner-manual functions deploy). All 8 (+5 batch-2 = 13) Earth layers + live Health Score + two-score Globe/Data View + nullschool-parity 4 slices merged + PWA + all-layers-live source-swap all live at `5df0df4`. `public-access-flip` (`05d00d2`) HELD — deploys at public-flip gate only. ✅ Both rescissions FIXED + deployed (`5df0df4`). ✅ P0 callable gate FIXED (`2ac0251`) — deployed at `5df0df4`. Public site-flip = FINAL gate (Fable + owner). Goal: 6/26._
...
| **8-Layer slice 3** (`fe87c3c`) | merged to main | green | ✅ deployed (`a49a957`) |
| **8-Layer slice 4** (`fbb4dd6`) — OSCAR refresher (`1ed1b6b`) + scalar refreshers (`e13c1e7`) merged; functions bundle deploy-ready | merged to main | green | ✅ deployed (`a49a957`) |
| **8-Layer slice 5** (`5ce9513`) — forest + human-density scalar; layer count 3→5 | merged to main | green | ✅ deployed (`a49a957`) |
...
| **Mini-player** (`4333563`) — shell-above-Navigator + PiP/MediaSession (Relax+Vibe survives route change) | merged to main | green | ✅ deployed (`a49a957`) |
| **Mini-player handoff** (`731b47a`) — single-instance handoff: full↔mini↔PiP | merged to main | green | ✅ deployed (`a49a957`) |
| **Filter panel fix** (`4c4cd80`) — earth+ filter panel fits without scroll; no globe-zoom hijack | merged to main | green | ✅ deployed (`a49a957`) |
| **8-Layer slice 6** (`d91da59`) — wildfire + biodiversity POINT layers (count 5→7) | merged to main | green | ✅ deployed (`a49a957`) |
| **Health Score function** (`58feb9f`) — `earthHealthScoreRefresh` — live Global Health Score aggregation (earth.healthscore.v1); reads cached grids → per-region + global; writes public Storage object | merged to main | green | ⏳ functions deploy PENDING owner |
| **8-Layer slice 7** (`299e9e6`) — SST ocean scalar layer (count 7→8) | merged to main | green | ✅ deployed (`a49a957`) |
| **SST anomaly update** (`353a478`) — `earthSstRefresh` emits SST ANOMALY vs 1991–2020 baseline | merged to main | green | ⏳ functions deploy PENDING owner |
| **✅ Rescission 2 FIXED** — `79ba5fa` erroneous removal reversed by `183c2f0`; `flights` + `ships` restored to governed `futureLayerIds` as Pro-tier aggregate layers; annotated per governance amendment session 24 | merged to main | green | ✅ live (`5df0df4`) |
| **Live Health Score UI capstone** (`855e6e0`) — reactive recompute on earth+ filter; replaces static 78; AIEDS as separate chip | merged to main | green | ✅ deployed (`a49a957`) |
| **LayerLegend + palette contract** (`d182d7a`) — reusable color-key widget | merged to main | green | ✅ deployed (`a49a957`) |
| **Layer-validation harness** (`ff22b06`) — reusable QA harness for Earth layers | merged to main | green | ✅ deployed (`a49a957`) |
| **Version-update banner** (`8df7ba2`) — in-app banner polls version.json + SW skip-waiting refresh | merged to main | green | ✅ deployed (`a49a957`) |
...
| **Globe-polish (2)** (`dd8f79a`) — auto-rotate + subtle highlight to selected region | merged to main | green | ✅ deployed (`a49a957`) |
| **Globe-polish (3)** (`f8b8589`) — subtle filter-change flourish | merged to main | green | ✅ deployed (`a49a957`) |
| **Globe-polish (4)** (`f52cafb`) — "i" info icon by the score estimation | merged to main | green | ✅ deployed (`a49a957`) |
| **Globe-polish (6)** (`d646bbe`) — Earth View taco counters | merged to main | green | ✅ deployed (`a49a957`) |
| **Globe-polish (7)** (`ab077de`) — Data View Layers reorganized to the 8 implemented layers | merged to main | green | ✅ deployed (`a49a957`) |
| **Globe-polish (5)** (`bd872d2`) — click a point marker for a governed data snapshot | merged to main | green | ✅ deployed (`a49a957`) |
| **Connect: BYOK providers** (`d76e455`) — BYOK providers status green in Connect + Add+ custom sources | merged to main | green | ✅ deployed (`a49a957`) |
| **Governance overlay-readiness fix** (`4b7e683`) — 8-layer overlay readiness corrected; all 8 layers now animate correctly | merged to main | green | ✅ deployed (`a49a957`) |
| **PWA Install** (`52fd725`) — installable on all devices (manifest + beforeinstallprompt); iOS apple-touch-icon + apple-mobile-web-app meta tags | merged to main | green | ✅ deployed (`a49a957`) |
| **GlobeView/DataView polish + shell + filter + region laser + OSCAR grid + ocean fixes** (`862645a`–`a49a957` — 22 commits: build_number stamp + SW-hash fallback; region highlight = animated border laser `0a81a82`; denser OSCAR 2° 180×91 grid `e75039f`; GlobeView polish 1+2+5+6+7+8 `d96e31b`/`1f3d093`/`6c921cb`/`f5727c4`/`bb7baa5`/`fc660f6`; DataView polish 1+2+3+6 `2946361`/`1ea1fd5`/`f0f1751`; shell banner + denser nav + install 90d gate `aa8ca0f`; all-layers-live data prep scalar grids `5b5c0aa`; filter polish + overlay state machine + Data View score fix + lamp mode + Globe View layout `b768db9`/`5c0c0a5`/`ea127f1`/`be94d65`/`fc62f6f`/`6a12e39`/`95104b9`; harden ocean mask + self-heal flow trail `a49a957`) | merged to main | green | ✅ deployed (`a49a957`) |
| **Point snapshots + Carbon/Glaciers functions + timeline=Now-backed** (`e6da040`) — wildfire/biodiversity point snapshot callables; Carbon/Glaciers data sources; timeline windows backed by Now | merged to main | green | ✅ live (`5df0df4`) |
| **Filter-state contract + timeline activation** (`2c925d4`+`96d6b60`) — My Location, layer SET/All-Layers, active timeline contract; activate timeline only by data backing (disable unbacked windows) | merged to main | green | ✅ live (`5df0df4`) |
| **All-layers-live source-swap** (`60ab9c7`) — 5 flips to live readers; all 8 layers now serve live data from callable sources | merged to main | green | ✅ live (`5df0df4`) |
| **Batch-2 layer data functions + wire 5 animated layers** (`8164ea6`+`42e8e53`) — functions deploy: VCM points + Protected-Areas scalar + Tree-Time (derived); wire Carbon/Glaciers/VCM/Protected-Areas/Tree-Time animated | merged to main | green | ✅ live (`5df0df4`) |
| **Pro-tier aggregate layer labels + batch-2 App-live** (`13be178`+`f03e9a3`) — label flights/ships/satellites "Pro · coming soon"; batch-2 verticals App-live after `8164ea6` shipped grids | merged to main | green | ✅ live (`5df0df4`) |
| **✅ Rescission 2 FIXED** (`183c2f0`) — `flights` + `ships` restored to governed `futureLayerIds` as Pro-tier aggregate layers (reversal of `79ba5fa` erroneous Tier 3 citation; annotated per governance amendment session 24) | merged to main | green | ✅ live (`5df0df4`) |
| **✅ P0 callable gate FIXED** (`2ac0251`) — allowlisted non-domain testers now pass `runAgentActionPreview` entitlement check; callable-backed features unblocked for allowlisted users | merged to main | green | ✅ live (`5df0df4`) |
| **Two-score model split + shared widgets** (`e28cbb5`) — `EarthScoreModel` split into `GlobalScore`+`RegionalScore`; shared `GlobalRingWidget`/`RegionalRingWidget`; Hive-persist `EarthFilterPrefs`; rotate-to-user-location-on-load | merged to main | green | ✅ live (`5df0df4`) |
| **Two-score Globe View layout** (`91648a5`+`aecbd55`) — remove Globe-View tacos counters (redesign `91648a5`); TL = Global full-ring; TR = Regional half-ring + filter chips; BL = filters (timeline removed); BR = timeline selector; region laser only on explicit pick (P3-5) | merged to main | green | ✅ live (`5df0df4`) |
| **Data View v2** (`3767c77`+`48966d6`+`cac27ef`) — single scrollable page; live-score breakdown; multi-select layer filters; layer surfaces registry-derived (no hardcoded list); Data View footer (tacos link + 3 credits, no counters); dashboard-72 removed (consolidation locked) | merged to main | green | ✅ live (`5df0df4`) |
| **TWO-SCORE REDESIGN COMPLETE** (`5140751`) — consume Earth's shared two-score rings in Data View; full two-score system: Globe View TL/TR/BL/BR + Data View rings | merged to main | green | ✅ live (`5df0df4`) |
| **Nullschool-parity 1a** (`c8ff53f`) — bilinear scalar raster interpolation; smooth per-pixel rendering for scalar layers; `web/earth_scalar_field.js` updated | merged to main | green | ✅ live (`5df0df4`) |
| **Remove rubicon-carbon demo icon** (`11458d4`) — chore: remove demo app icon from branding assets | merged to main | green | ✅ live (`5df0df4`) |
| **Nullschool-parity 2+5a** (`01a27e7`) — base-map rivers/lakes vector overlay + overlay scale-bar widget (single combined commit) | merged to main | green | ✅ live (`5df0df4`) |
| **Nullschool-parity 1b** (`8a42ae4`) — finer 1° scalar grids + gzip refresher writes (functions); SST stays coarse; 0.5° post-launch | merged to main | green | ✅ live (`5df0df4`) |
| **Nullschool-parity 4 — filter taxonomy** (`37cf8d3`) — animate/overlay/annotation slot architecture; filter taxonomy redesign complete | merged to main | green | ✅ live (`5df0df4`) |
| **Redesign-polish-1** (`acdebb9`) — timeline dot-line connector; fixed equal-width scores; filter-gap close; scale-bar into panel | merged to main | green | ✅ live (`5df0df4`) |
| **Redesign-polish-2** (`2ecbadb`) — icon filter slots; old-style layer indicators; dual-radial score (Fallback A — visual design still being refined); tacos/credits removed from Globe View | merged to main | green | ✅ live (`5df0df4`) |
| **ci(wf80): Cesium Ion token injection** (`dbe6096`) — inject Cesium Ion token into staging build + token gate; preview now at wf80/randomknights-xyz with Cesium Ion fully active | merged to main | green | ✅ live (`5df0df4`) |
| **test: de-duplicate stale Layers-workspace facet assertion** (`f2fd20d`) — test fix for nullschool-parity slice 4 filter taxonomy assertions | merged to main | green | ✅ live (`5df0df4`) |
| **#3 Data View semantic colour** (`708a533`) — semantic colour pass for Data View score visuals | merged to main | green | ⏳ deploy PENDING next wf90 |
| **#4 localized-points densifier** (`531b168`) — densify representative biodiversity/glacier/vcm point layers (functions); `earthLocalizationRefresh` + point-cloud outputs | merged to main | green | ⏳ deploy PENDING next wf90 + owner-manual functions deploy |
| **#1 polish-9 — scalar overlay OLD LARGE tiles** (`0afea74`) — INSUFFICIENT: screen-space raster (~5° fixed grid) pixelates at zoom and clips at globe limb | merged to main | green | ⚠ **SUPERSEDED by polish-10** (`04b4f0a`) — do NOT use raster approach; see binding lesson |
...
| **Filter parity batch 1 — unlock all layers** (`ccb72b9`+) — unlock all filter layers; decouple Animate from mode; default overlay=SST; animations opt-in from Animate filter; opaque scalar overlay (land-fill hidden; base-bleed bands fixed); HD on by default; crisper 1536×768 scalar texture | merged to main | green | ⏳ deploy PENDING next wf90 |
| **Health score v0.2** (`0046781`) — retire synthetic carbon ppm scalar (`b966f0b`); consume real CAMS/tree-time/forest/REAL grid inputs in score computation | merged to main | green | ⏳ deploy PENDING next wf90 + owner-manual `earthHealthScoreRefresh` redeploy |
| **CMEMS ocean-currents worker** (`ad16eec`) — drop-in CMEMS (Copernicus Marine) replacement for OSCAR in earth-worker pipeline; `RANDOM_CMEMS_*` owner-registered | merged to main | green | ⏳ deploy PENDING next wf90 + earth-worker redeploy |
| **SST burden fix + SSTA overlay** (`ad7d286` + `4c22c06`) — score SST burden corrected vs 1991–2020 climatology baseline; SSTA overlay layer added; one `earthSstRefresh` deploy ships both SST + SSTA grids | merged to main | green | ⏳ deploy PENDING next wf90 + owner-manual `earthSstRefresh` redeploy |
| **EarthFrameResolver + 2D-canvas renderer + 2D/3D toggle** (`99eafc8` + `04884b3` + `0daaf35`) — renderer-agnostic `EarthFrameResolver`; togglable 2D-canvas renderer (feature-flagged); pack5: 2D/3D toggle added to Globe View; **default → 2D** | merged to main | green | ⏳ deploy PENDING next wf90 + owner device-pass |
| **5 additive pointset layers + searchable browsers** (`525198b` `69d949e` `62998b8` `90187c4`) — register species/business/datacenter/industrial/protected-points pointsets; searchable per-record Data View browsers; filter-specific marker snapshots + unified layer icons; in-popup browse for clustered markers (no Data View bounce) | merged to main | green | ⏳ deploy PENDING next wf90 |
| **BAA ocean layer + antimeridian seam fix** (`7e7cd00` + `76061c3`) — NOAA Coral Reef Watch Bleaching Alert Area (BAA) ocean scalar layer; wrap scalar de-seam blur at antimeridian (fixes BAA 3D seam) | merged to main | green | ⏳ deploy PENDING next wf90 |
| **2D/3D toggle lifecycle fix** (`39e1ca2` → `b9df4c8` → `b6a5c83` → `99e351f`) — globe-disappear-on-toggle (keep-both-mounted → registerView-is-the-fix → pure lifecycle fix); blank 2D globe fix (platform-view factory registered BEFORE build); one-renderer-at-a-time enforced | merged to main | green | ⏳ deploy PENDING next wf90 + owner device-pass |
| **Rotate toggle + liveReady manifest + HD terrain** (`a0c712d` `38f436b` `8bd692e` `9fce6ec` `5aa3936`) — Rotate toggle restored to earth+ Control row; **liveReady manifest** gates undeployed live-grid fetches (no 403 spam on staging); HD Cesium terrain experiment; default 3D overlay=OFF (terrain shows through); OpenWeather One Call API 3.0 upgrade | merged to main | green | ⏳ deploy PENDING next wf90 + owner device-pass |
| **Score v0.3 — S1/S2/S3/S4** (`d6b4823` `1e2a1fb` `8560be9` `7cc8911` `89cd375` `ab15e4c`) — panel reconciliation + camelCase labels; S2 de-double-count via domains + anchored normalizers; map globe layers to v0.3 domains; Data View multi-select labels; S3 regional applicability mask + honest trend + global-weighting label; S4 ratified headline guards + weight-sensitivity disclosure | merged to main | green | ⚠ **CLIENT MERGED; `earthHealthScoreRefresh` function redeploy PENDING** — v0.2 function still deployed; **SST=0 symptom** active (live score SST domain reads stale from old function) until owner runs `firebase deploy --only functions:earthHealthScoreRefresh` |
| **earth2d composite + region/laser/my-location + spin polish** (`4979ef8` `5a8e0cf` `86b9e68` `1bcbdbe` `963d3d9` `bcc4dd3` `b42b33a`) — composite all 3 slots every frame; region focus/laser + my-location bridged to 2D stage; wheel zoom; pure-black background; frame-rate-independent spin (matches 3D speed); wildfire-weighted cluster dot polish | merged to main | green | ⏳ deploy PENDING next wf90 + owner device-pass |
| **Waves animate layer + earth2d quality + filter provenance** (`93e7fb5` `51c3d04` `6b81b2f` `06e0b24` `64d6761`) — NOAA WaveWatch III Waves flow layer (3rd flow layer; tsunami icon + full catalog/governance); nullschool-quality earth2d flow (denser, ocean animates, sea-masked trails); freeze 2D zoom/drag during overlay; wire Source row to active layers' real provenance | merged to main | green | ⏳ deploy PENDING next wf90 + owner device-pass |

`origin/main` (xyz) is at `64d6761` (git-verified, session 49). Hosting = `5df0df4` (deployed; wf90 2026-06-18). All commits from `a6441fb` through `64d6761` PENDING next wf90 + owner device-pass + earth-worker redeploy + `earthHealthScoreRefresh` function redeploy.
...
✅ **External-access gate (live):** `external_access_allowlist` RC key live + fail-closed. Domain OR allowlist; verified-only; Access ≠ Owner. Owner device-pass ✓ DONE (session 37). **Add-a-tester procedure:** (1) add email to `external_access_allowlist` in RC; (2) owner gate device-pass — allow allowlisted ✓ / block non-allowlisted ✓ / domain ✓; (3) tester all-clear. ✅ **P0 callable gate FIXED (`2ac0251`):** allowlisted non-domain testers now pass `runAgentActionPreview` entitlement check. Callable-backed features unblocked for allowlisted users. Pending deploy at next wf90.
...
✅ **P3 DEPLOYED (session 45):** P3 bundle + all subsequent commits through `a49a957` confirmed deployed via owner wf90. All P3+8-layer+capstone+globe-polish+PWA now live on prod. Functions deploy (`earthHealthScoreRefresh` + `earthSstRefresh`) PENDING owner manual deploy — separate from hosting deploy.
...
⚠ **Inspect DataSource Explorer — POST-LAUNCH (session 43):** read-only adapters; E1–E4; owner/org-gated; Recycler owner-only; secrets excluded at query layer. Off-Hive (Firestore). Renamed spec: [`internal/architecture/inspect-data-explorer-spec.md`](internal/architecture/inspect-data-explorer-spec.md). `data-explorer-v1-spec.md` superseded.

⚠ **§22 worktree-lane isolation (session 42, binding):** `apps/rand0m` = Earth/main integration ONLY. Every other lane in `worktrees/<lane>`. No branch-swap in `apps/rand0m`. Push→Earth-integrates→others-pull. See [`internal/automation/agent-coordination-standards.md`](internal/automation/agent-coordination-standards.md) §22.

⚠ **§23 contract-ownership (session 43, binding):** renderer contracts, model types, and data-shape contracts defined ONCE by Earth in `lib/models/earth/`. All other agents import — never copy or redefine. See coordination standards §23.

⚠ **Inspect security gate (session 43, `86ae546`):** Inspect page now redacts secrets and is gated to owner/org only (pre-public safety gate). Required before public launch. Deploy PENDING owner wf90.

✅ **Monorepo cleanup P1 (`fe44868`) + BOTH RESCISSIONS FIXED:**
1. ✅ Rescission 1 FIXED — `technologia.mp3` restored (`7b8c5c2`) + What's-New + alert/launch sound wired (`76809d7`). Deployed at `a49a957`. See [`internal/architecture/monorepo-cleanup-audit.md`](internal/architecture/monorepo-cleanup-audit.md) Rescission 1.
2. ✅ Rescission 2 FIXED — `flights`/`ships` restored to `futureLayerIds` as Pro-tier aggregate layers (`183c2f0`; reversal of `79ba5fa`). Pending next wf90. See Rescission 2.

⚠ **`public-access-flip` (`05d00d2`) HELD:** branch exists but is NOT on `origin/main`. Deploys at public-flip gate only — do not merge before Fable + owner sign-off.

⚠ **Governance amendment — Pro-tier gate for flights/ships/satellites (session 43):** Tier 2 aggregate density layers are Pro-tier only at launch. Updated in [`../READMORE/architecture/earth/human-activity-governance-amendment.md`](../READMORE/architecture/earth/human-activity-governance-amendment.md).

⚠ **BYOK ≥1-key gate (session 43):** unkeyed providers are disabled (not just quota-limited). `keySource=missing` → provider blocked. Detailed in [`internal/decisions/0006-byok.md`](../decisions/0006-byok.md).
...
| --- | --- | --- | --- |
| **Flow-field** | `earth_flow_field.js` · `EarthWindGrid` · `syncFlowField` | Wind (L1), Ocean OSCAR (L2) | ✅ Live (ratified + shipping) |
| **Point / scalar** | `earth_point_field.js` · `EarthPointGrid` · `syncPointLayer` | Air quality (L3), Forest (L4), Density (L5), Wildfire (L6), Biodiversity (L7), SST (L8) | ✅ Live (`451f7c2` + slices 3–7) |
...
| --- | --- | --- | --- | --- | --- |
| L1 | **Wind (GFS live)** | Flow-field | `b7f9849` + Phase 1b | ✅ **Live** | NOAA-GFS; `earthWindGfsRefresh` deployed; static-rep Phase 1a → live Phase 1b |
| L2 | **Ocean currents (OSCAR)** | Flow-field | `1ed1b6b` | ✅ **Live** | Phase 2 live OSCAR; mirrors GFS data contract; ocean-only geo-validity |
| L3 | **Air quality (Open-Meteo)** | Point / scalar | `fe87c3c` (rep), `fbb4dd6` (live) | ✅ **Live** — selectable rep/live | `earthAirQualityRefresh` deployed; count-based flip for Health Score |
| L4 | **Forest cover (Hansen/GLAD)** | Scalar | `5ce9513` | ✅ **Live** | Asset-backed; `earthForestRefresh` deployed |
| L5 | **Human density** | Scalar | `5ce9513` | ✅ **Live (representative)** | `earthHumanDensityRefresh` deployed; 0.10 weight provisional |
...
- ✅ **wf90 DEPLOYED `5df0df4` (2026-06-18):** rand0m.ai + ai-rand0m.web.app = 200. Allowlist-gated. `public-access-flip` (`05d00d2`) HELD — public-flip gate only.
- ✅ **Both rescissions FIXED:** deployed at `a49a957`/`5df0df4`. No open Earth reversal work.

**⚠ BLOCKING — function redeploys (owner-manual; wf90 is HOSTING-ONLY):**
- **`earthHealthScoreRefresh`** — v0.3 methodology merged on client (`ab15e4c`); **v0.2 function still deployed**. SST=0 symptom active: live score reads stale SST domain from old function. Owner runs `firebase deploy --only functions:earthHealthScoreRefresh` after `git pull`.
- **`earthSstRefresh`** — ships SSTA grid alongside SST (one deploy covers both); pending.
- **`earthWindGfsRefresh`** — THREDDS migration (`a6441fb`); pending.
- **`earthLocalizationRefresh`** — localized-points densifier (`531b168`); pending.
- **`gcloud run jobs deploy earth-worker`** — CAMS PM2.5/NO2 + REAL tree-time + REAL forest flip live on this deploy.

**Staging — pending next wf90 + owner device-pass:**
- 2D/3D toggle + lifecycle fix; earth2d composite/region/laser; Waves layer; filter parity batch 1; BAA/SSTA overlays; score v0.3 client; liveReady manifest; 5 pointset layers; all visual/perf polish from sessions 48–49.

**Open items:**
- **earth2d slot-parity** — composite + region/laser/my-location MERGED (`5a8e0cf` `1bcbdbe`); remaining slot-parity on branch (Earth agent).
- **Two-channel submit** (GitHub Discussions + Pro Test) — on branch; **owner GitHub App setup pending** before branch can merge. See architecture note: [`READMORE/architecture/earth/community-pro-submit-architecture.md`](../READMORE/architecture/earth/community-pro-submit-architecture.md).
- **Owner (blocking — orphan dirs):** `rm -rf rand0m-dvmerge rand0m-mainmerge` after confirming no process holds them.
- **Fixes agent:** delete merged branches (P3 bundle + C16 deletables); clean generated-registrant nit in `047cca8`; VCM test failures (3 pre-existing) still open.

**Data-View audit checklist (session 49):**
- [x] Score panel reconciliation + camelCase category labels (`d6b4823`)
- [x] Multi-select labels match v0.3 domains (`7cc8911`)
- [x] Searchable per-record browsers for 5 new impact layers (`69d949e`)
- [x] Filter-specific marker snapshots; unified layer icons Earth/Data View (`62998b8`)
- [ ] SST anomaly value correct in Data View score breakdown — **BLOCKED on `earthHealthScoreRefresh` redeploy** (SST=0 until then)
- [ ] SSTA overlay visible in Data View layers list — **BLOCKED on `earthSstRefresh` redeploy**
- [ ] Waves layer appears in Data View Animate slot — pending next wf90
- [ ] BAA layer appears in Data View overlay slot — pending next wf90
- [ ] earth2d full slot parity (all filter slots render in 2D) — on branch

> **FABLE CALLOUT:** Review: (1) BYOK ≥1-key gate + unkeyed-providers-disabled addition to ADR 0006 — confirm binding. (2) Governance amendment Pro-tier gate for Tier 2 aggregate density (flights/ships/satellites) — confirm binding. (3) `technologia.mp3` wiring triggers (launch + alert sound) — confirm scope before Earth implements. (4) `inspect-data-explorer-spec.md` ratification — owner/org gate, Recycler owner-only, off-Hive binding.
...
1. **Earth agent:** resolve two rescissions (restore `technologia.mp3`; re-add `flights`/`ships` to futureLayerIds as Pro layers)
2. **Owner:** `RECAPTCHA_V3_SITE_KEY` Actions var set → wf90 at `fe44868` → byte-hash delivery confirmed (all layers + capstone + Inspect gate)
3. **Owner:** `firebase deploy --only functions:...` (after `git pull`) — includes `earthHealthScoreRefresh` + updated `earthSstRefresh` + all refresh functions
4. **Fable:** record deployed SHA (git-verified from owner confirm)
5. **Monitoring period:** App Check MONITORING baselines legitimate traffic
6. **Earth + Owner:** enforce-flip (`6fd5b60`) + Pro/managed server tier-read → SECOND functions deploy (Fable confirms whether `6fd5b60` rides initial batch or holds)
7. **Owner:** deploy `public-access-flip` (`05d00d2`) — HELD; deploys at this gate only
8. **Owner device-pass:** Health Score live (not static 78) ✓ + AIEDS as separate chip ✓ + Disclosure Safeguards ✓ + entitlement checks ✓ + non-domain Google sign-in ✓ + Inspect gated to owner ✓
9. **Test agent:** callable gate mismatch P0 fixed (entitlement check replaces domain-only guard)
10. **Fable:** legal sign-off (device-pass review)
11. **Owner:** PUBLIC SITE-FLIP
...
- **RESCISSION fix (unblocked):** restore `technologia.mp3` + re-add pubspec audio glob. Can land before or alongside anim-bugfix — file-disjoint.
- Gate each before next opens.

### Earth Pro track (post-launch; Fable spec per vertical required)

Earth Pro = entitlement-gated features for Pro/org subscribers.

| Track | Description | Gate |
| --- | --- | --- |
| **All-Layers mode** | Show all 8 layers simultaneously (Pro-only toggle) | Fable spec + Pro entitlement gate |
| **HD assets** | Higher-resolution data assets for Pro subscribers | Fable spec required |
| **Flights density (aggregate)** | Tier 2 aggregate; identity-suppressed; ≥24h delay; governance spec per `human-activity-governance-amendment.md` | Separate Fable governance spec |
| **Ships density (aggregate)** | Tier 2 aggregate; same constraints as flights | Separate Fable governance spec |
...
| Batch | Scope | Status |
| --- | --- | --- |
| **1 — Filter unlock** | Client-only: unlock all layers except Space; Animate=Wind/Currents/Waves in every mode; default overlay=SST | ✅ **MERGED** (`ccb72b9`+; session 49) — pending next wf90 deploy |
| **2 — CMEMS currents + Waves** | Worker: CMEMS Copernicus Marine currents pipeline + Waves (HTSGW) earth-worker job | 🔄 **PARTIAL** — CMEMS worker merged (`ad16eec`); Waves layer merged (`93e7fb5`); earth-worker CMEMS job pending owner redeploy |
| **3 — SSTA/BAA overlays** | SST Anomaly (SSTA) + Barometric Pressure Anomaly (BAA) overlay layers | ✅ **MERGED** — SSTA (`4c22c06`); BAA (`7e7cd00`); function deploys pending |
| **4 — Projection toggle** | 2D/3D toggle (Cesium-native 3D↔2D↔Columbus; no d3 projections) | ✅ **MERGED** (`0daaf35`; default→2D; 3D available via toggle) — pending next wf90 deploy |
...
### Multi-Platform — PWA Install ✅ SHIPPED (`52fd725` — merged to main; deploy PENDING wf90)
...
- **Connect: BYOK providers** (`d76e455`) — BYOK providers status green in Connect + Add+ custom sources surface. ✓ (merged; deploy PENDING wf90)
- **Globe-polish 1–7** (`53a64c3`→`bd872d2`) — (1) markers pinned to globe (Cesium entities); (2) auto-rotate + subtle region highlight; (3) subtle filter-change flourish; (4) "i" info icon by score estimation; (5) click point marker for governed data snapshot; (6) Earth View taco counters; (7) Data View Layers reorganized to 8 implemented layers. ✓ (merged; deploy PENDING wf90)
- **Rescission 1 FIXED** (`7b8c5c2` restore + `76809d7` wiring) — `technologia.mp3` restored from git history + `assets/audio/` glob re-added; What's-New section + alert/launch sound wired; `web/release-notes.json` introduced. ✓ (merged; deploy PENDING wf90)
- **8-LAYER PROGRAM COMPLETE** (session 43) — all 8 layers live at `fe44868`. L1 Wind (`b7f9849`), L2 Ocean OSCAR (`1ed1b6b`), L3 Air quality (`fbb4dd6` live), L4 Forest (`5ce9513`), L5 Density (`5ce9513`), L6 Wildfire (`d91da59`), L7 Biodiversity (`d91da59`), L8 SST anomaly (`299e9e6`+`353a478`). Renderer contracts ratified+shipping. ✓ (merged; deploy PENDING wf90)
- **Live Health Score CAPSTONE** (session 43) — `earthHealthScoreRefresh` (`58feb9f`) live server aggregation; `855e6e0` reactive UI recompute on filter; replaces static 78; AIEDS separate chip. SST anomaly updated (`353a478`). earth.healthscore.v1. ✓ (merged; functions deploy PENDING owner)
- **Inspect security gate** (`86ae546`) — secrets redacted; Inspect page gated to owner/org (pre-public safety). ✓ (merged; deploy PENDING wf90)
- **Version-update banner** (`8df7ba2`) — in-app banner polls version.json + SW skip-waiting. ✓ (merged; deploy PENDING wf90)
- **Mini-player** (`4333563` + `731b47a`) — persistent mini-player (Relax+Vibe survives route change) + single-instance handoff (full↔mini↔PiP). ✓ (merged; deploy PENDING wf90)
- **Monorepo cleanup P1** (`fe44868`) — 7 unused Earth textures deleted; README de-four-app'd. ⚠ Two rescissions OPEN (technologia.mp3; flights/ships futureLayerIds). ✓ (merged; deploy PENDING; rescissions pending Earth reversal)
- **§23 contract-ownership BINDING** (session 43) — contracts defined ONCE by Earth in `lib/models/earth/`; all agents import. Codified in coordination standards §23. ✓
- **Inspect DataSource Explorer spec** (session 43) — [`internal/architecture/inspect-data-explorer-spec.md`](internal/architecture/inspect-data-explorer-spec.md); replaces `data-explorer-v1-spec.md`. POST-LAUNCH, owner/org, off-Hive, Recycler owner-only. ✓
- **Governance amendment — Pro-tier gate for Tier 2 density** (session 43) — flights/ships/satellites aggregate density layers = Pro-tier only. Updated [`../READMORE/architecture/earth/human-activity-governance-amendment.md`](../READMORE/architecture/earth/human-activity-governance-amendment.md). ✓
- **BYOK ≥1-key gate** (session 43) — unkeyed providers disabled (not just quota-limited); updated [`internal/decisions/0006-byok.md`](../decisions/0006-byok.md). ✓
- **Monorepo cleanup audit** (session 43) — [`internal/architecture/monorepo-cleanup-audit.md`](internal/architecture/monorepo-cleanup-audit.md); P1 done; two rescissions documented; P2/P3 pending Fable. ✓
...
- **8-Layer slices 1–5 merged** (`90eb743` / `451f7c2` / `fe87c3c` / `fbb4dd6` / `5ce9513`). OSCAR refresher `1ed1b6b` + scalar refreshers `e13c1e7` + enforce-flip staged `6fd5b60`. Functions bundle deploy-ready. ✓ (merged; deploy PENDING owner)
...
- **Stale correction — batch-2 grids 403 (session 45):** batch-2 refresher functions had a 403 on prior deploy due to stale local checkout (same root cause as wind/air-quality miss in session 36). Refresher functions EXIST on the remote — fix is redeploy by refresher name only: `firebase deploy --only functions:<refresher-name>` after `git pull`. Not a code issue; not a missing deploy; do not create new function code.
...
- **8-Layer Program COMPLETE (session 43):** all 8 layers merged at `fe44868`. Renderer contracts ratified (flow-field + point/scalar both shipping; point shipping as `earth_point_field.js` not `earth_point_renderer.js`). Prior "FABLE CALLOUT — ratification pending" language superseded.
- **Live Health Score replaces static 78 (session 43, `855e6e0`):** prior "static 78 - viewer-AI-carbon estimate" is retired. Canonical score is now `earthHealthScoreRefresh`-aggregated + client-reactive. AIEDS digital footprint surfaced as a separate chip — never blended.
- **✅ `79ba5fa` RESCISSION 2 FIXED (session 45, `183c2f0`):** Earth agent restored `flights` + `ships` to governed `futureLayerIds` as Pro-tier aggregate layers; annotated per governance amendment session 24. Reversal is on main (`183c2f0`); pending next wf90 deploy.
- **`79ba5fa` RESCISSION context (session 43):** commit had cited Tier 3 prohibition (live per-vehicle tracking) to justify removing `flights`/`ships` from `futureLayerIds`. Tier 2 aggregate density is PERMITTED per governance amendment session 24. Now corrected.
- **`fe44868` RESCISSION — `technologia.mp3` deletion was premature (session 43):** deleted as "zero refs" but is the intended launch/alert sound. Wiring is pending (not abandoned). Earth agent must restore from git history. Playback trigger scope requires Fable confirmation before wiring.
- **`public-access-flip` (`05d00d2`) HELD (session 43):** SHA exists on origin but is NOT in `origin/main`. This is the gate-controlled public access flip. Must NOT be merged before Fable + owner sign-off at public-flip checkpoint. Do not confuse with the wf90 hosting deploy.
- **What's-New release-notes source (session 44 — updated):** `76809d7` (Rescission 1 wiring) introduced `web/release-notes.json` as the authoritative in-app release-notes source. Source is now established. Post-launch: owner directive + design spec for ongoing content maintenance of `release-notes.json`.
- **`rk_media` adopt-vs-sunset (session 43 — queued):** `rk_media` package decision post-launch. If adopted → formalize as governed package; if sunset → consolidate into `apps/rand0m`. No action before launch.
- **`connect_page_test` refactor (session 43 — queued):** test structure reflects pre-retirement Connect agent architecture. Post-launch; Test agent owns.
...
_Updated session 49 (git-verified). qa-kitt/.github: 2 branches (main + readless-readmore-reorg). xyz: `origin/main` = `64d6761` (2D/3D toggle + lifecycle fix; score v0.3 client; filter parity batch 1; CMEMS/SSTA/BAA/Waves/earth2d on main; hosting = `5df0df4`). All 8 layers + Health Score + both rescissions + two-score + nullschool-parity + filter parity batch 1 + earth2d renderer on main. `public-access-flip` (`05d00d2`) is NOT on main — HELD branch. Two-channel submit on branch (not merged)._
...
| `feature/public-access-flip` (`05d00d2`) | xyz | **HELD — NOT on main** | Public-flip gate; deploys only at public site-flip checkpoint; do not merge before Fable+owner sign-off |
...
_Updated session 49. `origin/main` = `64d6761`. Deployed = `5df0df4` (wf90 2026-06-18). Go/no-go detail: [`internal/architecture/public-launch-readiness.md`](internal/architecture/public-launch-readiness.md)._
...
| **wf90 deploy** | ✅ **DEPLOYED `5df0df4`** | wf90 confirmed at `5df0df4` (2026-06-18). Next wf90 PENDING at `64d6761` tip (filter parity + 2D renderer + score v0.3 client + BAA/SSTA/Waves + all sessions 48–49 work). Owner device-pass required + function redeploys (`earthHealthScoreRefresh`, `earthSstRefresh`, `earthWindGfsRefresh`, `earthLocalizationRefresh`) + earth-worker redeploy. |
...
| **P0 — callable gate mismatch fix** | ✅ **FIXED (`2ac0251`)** | `runAgentActionPreview` entitlement check wired; allowlisted non-domain users unblocked. Pending next wf90 deploy. |
...
| **`public-access-flip` (`05d00d2`) deploy** | ⛔ **HELD — public-flip gate only** | Merge + deploy at public site-flip checkpoint only. Do not trigger before Fable+owner sign-off. |
