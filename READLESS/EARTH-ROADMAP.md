...
4. **Earth agent:** implement `earth/scientist-session-continuity` — Phase 0 `earth_tab.dart` extraction (2,387 lines → CODEX threshold) + session-continuity spec. See [`architecture/scientist-session-continuity-spec.md`](architecture/scientist-session-continuity-spec.md).
...
- **`earth_tab.dart` architecture threshold:** 2,387 lines as of `01a070a` — exceeds CODEX 2,000-line architecture-review threshold. Phase 0 extraction (widget/service split) scheduled as first task in `earth/scientist-session-continuity`. Do not add further features to `earth_tab.dart` before extraction.
- **`liveProviderEnabled` kill switch:** compile-time flag. Toggling requires a redeploy; it is not a runtime feature flag. Account for this in any live-provider rollout plan.
